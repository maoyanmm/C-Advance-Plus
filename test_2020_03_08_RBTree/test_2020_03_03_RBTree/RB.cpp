...
namespace RBtree
{
	...
	private:
		...
		//验证二叉树
		bool _Is_RBtree(pNode cur, int nowCount, const int& blackCount)
		{
			if (cur == nullptr)
			{
				if (nowCount == blackCount)
				{
					return true;
				}
				else
			...
			if (cur->_colour == RED && cur->_parent->_colour == RED)
			{
				return false;
			}
			return _Is_RBtree(cur->_left, nowCount, blackCount) && _Is_RBtree(cur->_right, nowCount, blackCount);
		}
	public:
		RBTree()
		{
		...
		bool Is_RBtree()
		{
			...
			return _Is_RBtree(root, 0, blackCount);
		}
	};
...
//	}
//	rb.In_Order();
//	std::cout << rb.Is_RBtree() << std::endl;
//
//}
//
//int main()
